...
def make_app():
    ...
    def home(post_id):
        ...
        track = track.drop(['artist_name', 'track_id', 'track_name',
                            'track_index_num', 'album_cover_url'], axis=1)
        preds = model.kneighbors(track)
        # app = Flask(__name__)
        df = songs100[(songs100['track_index_num'] == preds[1][0][0]) | (songs100['track_index_num'] == preds[1][0][1]) | (
            songs100['track_index_num'] == preds[1][0][2]) | (songs100['track_index_num'] == preds[1][0][3]) | (songs100['track_index_num'] == preds[1][0][4])]
        dict_set = [{
            'track_index_num': x[0],
            'track_id': x[1],
            'track_name': x[2],
